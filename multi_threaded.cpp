...
private:
    ...
    // Function to list directories recursively in a thread-safe manner
    void listDirectoryRecursive(const string& path, const vector<string>& options) {
        lock_guard<mutex> lock(mtx);
        listDirectoryRecursiveImpl(path, options);
    }

    // Recursive implementation of listing directories
    void listDirectoryRecursiveImpl(const string& path, const vector<string>& options) {
        try {
            for (const auto& entry : fs::directory_iterator(path)) {
                ...
                if (fs::is_directory(entry)) {
                    cout << " (Directory)";
                    // Create a thread to list the directory recursively
                    thread([this](const std::string& path, const std::vector<std::string>& options) {listDirectoryRecursive(path, options);}, entry.path().string(), options).detach();
                }
    ...
    void removeFileRecursive(const string& path) {
        try {
            for (const auto& entry : fs::directory_iterator(path)) {
                if (fs::is_directory(entry)) {
                    // Create a thread to remove the directory recursively
                    std::thread(&Shell::removeFileRecursive, this, entry.path().string()).detach();
                } else {
                    // Remove the file
                    fs::remove(entry);
                }
...
public:
    ...
    void listDir(const vector<string>& options, const vector<string>& args) {
        // Check for the presence of the -r option
        ...
        if (args.empty()) {
            ...
            if (recursive) {
                ...
                std::thread(&Shell::listDirectoryRecursive, this, current_directory, options).detach();
            } else {
                // Regular listing of the current directory
            ...
            for (const auto& arg : args) {
                if (recursive) {
                    // Call the recursive listing function only if -r is specified
                    std::thread(&Shell::listDirectoryRecursive, this, arg, options).detach();
                } else {
                    // Call the regular listing function
    ...
    void removeFile(const vector<string>& options, const vector<string>& args) {
        if (!args.empty()) {
            try {
                ...
                if (recursive) {
                    // Remove directory and its contents recursively
                    thread(&Shell::removeFileRecursive, this, args[0]).detach();
                } else {
                    // Remove file or empty directory
                    fs::remove(args[0]);
                }
